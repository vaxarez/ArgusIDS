...
static host_track_t *track_host(uint32_t ip) {
    ...
    host_track_t *h = &hosts[host_count++];
    memset(h, 0, sizeof(*h));
    h->ip = ip;
    h->window_start = time(NULL);
    h->syn_window_start = h->window_start;
    return h;
}

static icmp_track_t *track_icmp(uint32_t ip) {
...
int detector_analyze(const parsed_packet_t *pkt, alert_t *alert) {
    ...
    if (pkt->ip.protocol == 1) {
        icmp_track_t *ic = track_icmp(src);
        if (!ic)
            return 0;
    ...
    host_track_t *h = track_host(src);
    if (!h)
        return 0;

    if (is_suspicious_port(dst_port) && now - h->susp_cooldown > 30) {
